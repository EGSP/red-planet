...
Кто участвует в экономике: коммандер (собственный доход — без него первую электростанцию не на что строить), генераторы, месторождения (дают метал, едят энергию), каркасы, синтезатор метала (жжёт энергию ради метала — сброс излишков). Всё это одна группа `economy` и один интерфейс `IEconomyActor`.
...
**Носители оружия ведут себя одинаково.** Группа `armed`, интерфейс `IArmed` — и дальше всё решает `WeaponSystem`: враг, коммандер и турель проходят один и тот же порядок «остыл → есть цель → в радиусе → довернуть → в конусе → выстрел». Разница между ними только в ответах на два вопроса: `CanFire` (турель и враг стреляют всегда, юнит — без приказов или по приказу атаки) и `FireTarget` (враг бьёт того, к кому шёл; коммандер по приказу — назначенного; турель не выбирает, ей находят ближайшего).
