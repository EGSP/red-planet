...
///
/// СОБСТВЕННОЙ ЛОГИКИ НЕТ. Узел не знает ни владельца, ни события: когда проиграть,
/// решает <see cref="EffectSystem"/>.
///
/// В САМОЙ ПАРТИИ ЭТИ УЗЛЫ НЕ ЖИВУТ. Сцена эффекта разбирается однажды
/// (<see cref="ParticleBake"/>), и от неё остаются настройки рождения, а частицы выпускает
/// общее поле, заведённое на все эффекты с тем же материалом. Поэтому корень и его части
/// суть описание вида, которое художник правит и видит в редакторе, а не узлы, которые
/// работают в бою.
/// </summary>
[Tool, GlobalClass]
public partial class BurstParticles : Node2D
{
    ...
    /// где никакого выстрела не было. Проигрывает эффект только тот, кто знает событие,
    ...
    /// <summary>
    /// Проиграть эффект с начала. Служит предпросмотру в редакторе: в партии эффект
    /// проигрывается вбросом в общее поле, а не этими узлами.
    ///
    /// Именно <c>Restart</c>, а не включение выдачи: части объявлены одноразовыми
    /// (<c>one_shot</c>), и повторный показ обязан начинать вспышку заново, не дожидаясь,
    /// пока догорит прежняя.
    /// </summary>
    public void Play()
    {
        foreach (var emitter in Emitters)
