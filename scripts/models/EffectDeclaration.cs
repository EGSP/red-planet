...
/// <summary>
/// Общая часть узлов, объявляющих сцену в модели: ссылка на неё, множитель размера
/// и предпросмотр в редакторе. Объявляемое у наследников разное — гибель
/// (<see cref="DeathEffect"/>), попадание снаряда (<see cref="ImpactEffect"/>), сам снаряд
/// (<see cref="ProjectileDeclaration"/>), — а вот объявление у них устроено одинаково,
/// и повторять его в каждом означало бы, что правка предпросмотра идёт дважды.
///
...
/// что до события она не нужна. Поэтому узел объявляет только ссылку и размер; сцену
/// разбирает <see cref="EffectSystem"/>, а частицы выпускает общее поле.
///
/// СОБСТВЕННОЙ ЛОГИКИ НЕТ — как и у <see cref="ModelTool"/>, который не разыскивает своё
/// определение оружия. Узел не знает ни события, ни слоя эффектов, ни мира вокруг.
/// </summary>
[Tool]
public abstract partial class EffectDeclaration : Node2D
{
    ...
    /// эффект этого события, объявленный у <see cref="EffectSystem"/>.
    /// </summary>
    [Export]
