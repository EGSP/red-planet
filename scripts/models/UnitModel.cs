...
public partial class UnitModel : Node2D
{
    ...
    /// <summary>
    /// Объекты отрисовки, принимающие величины сущности: те, чей материал объявляет параметр
    ...
    /// <summary>Подчинены ли картинки материалу родителя. Нужно каркасу строительства.</summary>
    ...
    private void Build()
    {
        ...
        // тем же порядком: материал у них общий со всеми носителями вида
        CollectPainted(this);
        CollectStaged(this);
    ...
    /// стоит материал с шейдером, объявляющим параметр экземпляра <c>team_color</c>:
    /// остальное изображение остаётся таким, каким нарисовано.
    ///
    /// Цвет пишется объекту отрисовки, а не материалу: материал один на вид, и запись в него
    ...
    /// вида разная, поэтому материалом её задать нельзя вовсе: она идёт параметром
    ...
    /// <summary>
    /// Переключить всю модель на материал родителя. Нужно каркасу: шейдер строительства
    /// стоит на слое <see cref="BlueprintLayer"/>, а материал в Godot есть свойство узла
