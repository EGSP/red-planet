...
/// <summary>Род сущности с точки зрения выделения.</summary>
public enum SelectionGroup
{
    /// <summary>Ходячее небоевое: коммандер, фабрикаторы, копатели.</summary>
    Bots = 0,
    ...
    /// <summary>
    /// Боевые машины: вооружённые и без рабочего инструмента. Род выводится из снабжения,
    /// а не задаётся в файле, — см. <c>ContentCompiler.Link</c>. Коммандер сюда не входит,
    /// хотя и вооружён: он прежде всего строитель базы, и отделять его от фабрикаторов,
    /// с которыми игрок выделяет его вместе, значило бы мешать обычной застройке.
    /// </summary>
    Army = 2,
}
...
/// выделить отряд.
///
/// КАК. Первым делом отбирается армия: попал в рамку хоть один боевой — в выделении
/// остаются только боевые, сколько бы их ни было. Взвешивание при этом не применяется вовсе,
/// потому что приоритет армии безусловен, а вес выражал бы его как перевес, который можно
/// перебить числом. Обоснование — в <see cref="KeepDominant"/>.
///
/// Если боевых в улове нет, спор решают веса: побеждает род с наибольшим произведением
/// количества на вес. Веса подобраны так, чтобы при равном числе побеждали боты:
/// две постройки перевешивают одного бота, но не двух.
///
...
public static class SelectionGroups
{
    ...
    ///
    /// Армия во взвешивание не входит: её отбирают до него и безусловно. Вес у неё есть
    /// только затем, чтобы у каждого значения перечисления был ответ.
    /// </summary>
    ...
    /// останутся только боевые, и число их роли не играет. Рамка по своей базе почти всегда
    /// захватывает лишнее: боевые машины стоят среди фабрикаторов, генераторов и стен, —
    /// и игрок, тянущий рамку по такому месту, имеет в виду армию, а не застройку.
    /// Взвешивание здесь не годится, поскольку любой перевес перебивается числом: три
    /// фабрикатора отобрали бы у игрока два танка, хотя приказать фабрикаторам он не хотел.
    ///
    /// Обратное неверно: рамкой по одной застройке армия не выделяется, потому что боевых
    /// в улове тогда нет вовсе, и спор родов решают веса, как и прежде.
    /// </summary>
    public static void KeepDominant(List<IOrderable> actors)
    {
        ...
        if (actors.Exists(actor => actor.SelectionGroup == SelectionGroup.Army))
        {
            actors.RemoveAll(actor => actor.SelectionGroup != SelectionGroup.Army);
            return;
        }
