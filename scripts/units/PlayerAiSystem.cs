...
/// — ОГОНЬ И СБЛИЖЕНИЕ С ПРОТИВНИКОМ сюда не входят вовсе: незанятый вооружённый вступает
///   в бой сам, без приказа, — см. <see cref="Unit.OnIdle"/>. Прежде бой выдавался отсюда
///   приказом атаки, и цель искалась вокруг якоря, отчего юнит, сместившийся с якоря
///   в прошлом бою, переставал замечать противника рядом с собой.
///
/// РАБОТА ОГРАНИЧЕНА РАДИУСОМ ВНИМАНИЯ вокруг якоря (см. <see cref="IWorker.Anchor"/>),
/// а не вокруг текущего положения. Поэтому отлучка всегда возвращает исполнителя туда,
/// куда его послали, и цепочка «нашёл дело — отошёл — от новой позиции нашёл следующее»
/// больше не уводит отряд с рубежа. У боя предел устроен иначе: цель ищется вокруг самого
/// юнита, а якорь ограничивает лишь то, насколько далеко он вправе за ней отойти.
///
...
public partial class PlayerAiSystem : GameSystem
{
    ...
    private Order ChooseJob(IWorker worker)
    {
        ...
        // Бой сюда не входит: незанятый вооружённый вступает в него сам, без приказа,
        // по тому же правилу, что и в приказе «идти с боем» (см. Unit.OnIdle). Прежде бой
        // выдавался отсюда приказом атаки, и цель искалась вокруг ЯКОРЯ, отчего юнит,
        // сместившийся с якоря в прошлом бою, переставал замечать противника рядом с собой
