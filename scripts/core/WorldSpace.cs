...
///
/// ПОЧЕМУ РАСКЛАДОК ДВЕ, А НЕ ОДНА. Различаются они не размером клетки, а содержимым:
...
/// на каждом кандидате там, где сейчас его нет вовсе. Размер клетки у обеих один: он выбран
/// под расталкивание, а поиск ближайшего от мелкой клетки не страдает, поскольку прекращается
/// в первых кольцах.
///
...
{
    /// <summary>
    /// Сторона клетки. Вдвое больше клетки навигации: окрестность три на три покрывает
    /// радиус чутья при локальном обходе, ради которого размер и подбирался.
    /// </summary>
    public const int CellPx = Const.Unit * 2;
    ...
    {
        _index = index;
        Targets = new SpatialGrid<IDamageable>(CellPx);
        Mobiles = new SpatialGrid<IMobile>(CellPx);
    }
    ...
    public SpatialGrid<IDamageable> ReadyTargets()
    {
        ...
        {
            // Крупная цель кладётся во все клетки своего прямоугольника: иначе кольцевой
            // обход дошёл бы до её края раньше, чем до клетки центра, и не увидел ничего
