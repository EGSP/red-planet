...
/// или коммандера — поражение. Система появляется вместе с сессией и больше нигде не живёт.
///
...
/// и коммандер ещё могут пасть, и победа засчитывается только тем, кто дожил.
///
/// Счётчик порталов, а не привязка к одному id: если портал снесли во время ожидания,
/// отсчёт сбрасывается; если построили снова — начинается заново. Гибель крепости или
/// коммандера сразу завершает партию поражением, даже если отсчёт уже шёл.
/// </summary>
public partial class VictorySystem : GameSystem
{
    ...
    private void OnEntityDestroyed(EntityDestroyed record)
    {
        ...
        if (record.DefinitionId == FortressId || record.DefinitionId == CommanderId)
        {
