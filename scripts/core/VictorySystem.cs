...
/// <summary>
/// Победа и поражение: достроенный портал и минута удержания — победа; гибель коммандера —
...
/// а не кнопкой. Минута ожидания после достройки — цена удержания: враг ещё бьёт, коммандер
/// ещё может пасть, и победа засчитывается только тем, кто дожил.
///
/// ПОЧЕМУ ПОРАЖЕНИЕ ТОЛЬКО ПО КОММАНДЕРУ. Стартовой базы у игрока нет: она выполняла ту же
/// роль единственной уязвимой точки, но при этом привязывала игрока к месту высадки.
/// Коммандер подвижен, поэтому та же угроза не ограничивает выбор направления развития.
///
/// Счётчик порталов, а не привязка к одному id: если портал снесли во время ожидания,
/// отсчёт сбрасывается; если построили снова — начинается заново. Гибель коммандера
...
public partial class VictorySystem : GameSystem
{
    ...
    private void OnEntityDestroyed(EntityDestroyed record)
    {
        ...
        if (record.DefinitionId == CommanderId)
        {
