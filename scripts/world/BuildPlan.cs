...
///
/// ПЛАН — НЕ ПРИКАЗ, хотя и рождается из него: он сущность мира, а приказ на него —
/// обычный <see cref="OrderKind.Build"/>. Приказ живёт в очереди исполнителя, план — в мире,
/// и один и тот же план может быть поручен нескольким сразу.
///
/// НО ПЛАН БЕЗ ПОРУЧЕНИЯ НЕ ЖИВЁТ. Как только не остаётся ни одной очереди, в остатке
/// которой стоит приказ на этот план, план снимается (см. <see cref="ReleaseAbandoned"/>).
...
public partial class BuildPlan : Node2D, IWorkSite, IFacing, IFootprint
{
    ...
    /// <summary>
    /// Снять планы, до которых больше не дойдёт ни один исполнитель. Зовётся отовсюду,
    /// где исполнитель уходит из ветки: приказ взамен прежних, задача от системы выдачи,
    /// отделение части отряда и гибель.
    ///
    /// ПЛАН ЖИВЁТ РОВНО СТОЛЬКО, СКОЛЬКО ЖИВЁТ ПОРУЧЕНИЕ. Прежде план переживал гибель
    /// того, кому был назначен, в расчёте на то, что за него возьмётся кто-то другой.
    /// Браться было некому: подвижный строитель стройку сам не берёт, а неподвижный
    /// достаёт только то, что попало под манипулятор. Оставшийся план не исполнялся
    /// никогда и при этом продолжал занимать место для правила постановки.
    /// </summary>
    ...
    /// <summary>
    /// Дойдёт ли до плана хоть один исполнитель.
    ///
    /// СПРАШИВАЕТСЯ У ОЧЕРЕДЕЙ, А НЕ У ВЕТОК. Ветка жива, пока жив хоть кто-то выше
    /// по цепочке, но сам приказ при этом может остаться позади всех указателей: уборка
    /// срезает пройденное только с головы ветки и только при обычном шаге. Поэтому вопрос
    /// задаётся исполнителям поимённо — входит ли приказ в их остаток.
    /// </summary>
    private static bool Ordered(BuildPlan plan)
    {
        ...
        foreach (var actor in index.All<IOrderable>())
            if (actor.Orders != null && actor.Orders.Assigned(plan))
                return true;
