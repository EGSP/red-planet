...
public partial class BuildPlan : Node2D, IWorkSite, IFacing
{
    ...
    ///
    /// ПРИКАЗ НЕ ЗАМЕНЯЕТСЯ, А ДОПОЛНЯЕТСЯ. Приказ на каркас вписывается следом за приказом
    /// на план в каждую очередь, где тот значится, — и это ОДИН объект приказа на всех:
    /// состояние исполнения у него общее, поэтому подошедшие следом узнают, что работа
    /// уже начата, и включаются в неё, а не начинают её заново.
    ///
    /// Очереди перебираются разрезом индекса: с тех пор как список приказов стал сущностью,
    /// спрашивать их у исполнителей поимённо не нужно — и не следует, иначе в общий список
    /// приказ попал бы столько раз, сколько на него подписано юнитов.
    /// </summary>
    public void Realize()
    {
        if (_spent)
        ...
        var follow = Order.Work(OrderKind.Build, frame);
