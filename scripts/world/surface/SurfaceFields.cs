...
///
/// ТЕМПЕРАТУРА ВЫГРУЖАЕТСЯ РАДИ ОВЕРЛЕЯ. Отбор биомов и декалей по-прежнему читает
/// <see cref="TemperatureAt"/> на процессоре. Текстура нужна шейдеру только когда в
/// инспекторе включён <see cref="SurfaceSettings.ShowTemperature"/>: она показывает то же
/// поле, по которому идут биомы, поверх местности.
///
