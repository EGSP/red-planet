...
///
/// ПОЧЕМУ БАЗА НЕ ЗАПЕКАЕТСЯ. При <see cref="CameraSettings.ZoomNear"/> порядка 2.5 запечённая в
...
public partial class SurfaceRenderer : Node2D
{
    ...
    /// <summary>
    /// Снять материал перед сохранением сцены и собрать заново после.
    ///
    /// ЗАЧЕМ ЭТО НУЖНО. Редактор сохраняет всё, что лежит в свойствах узла, а в материале
    ...
    /// <summary>
    /// Завести материал, если его нет. Он создаётся всякий раз заново и в сцене не
    ...
    /// <summary>Снять материал и поля: после этого узлу нечего отдать сохранению сцены.</summary>
    private void Detach()
    {
        Material = null;
        _material = null;
