...
/// <summary>
/// Раскладка застройки: во что превращается протаскивание мыши с выбранной постройкой.
///
/// НЕ ПУТАТЬ С <see cref="BuildPlan"/>. Здесь считаются места — где и под каким углом
/// встанут будущие строения; планом же называется сущность мира, которая из принятого
/// места рождается. Расчёт был назван планом до того, как план появился в игре, и имя
/// пришлось освободить.
///
/// ЗАЧЕМ ОТДЕЛЬНЫМ КЛАССОМ И БЕЗ НОД. Раскладку спрашивают дважды и с разными намерениями:
/// призрак под курсором — каждый кадр, чтобы показать, и постановка планов — один раз,
...
public static class BuildLayout
{
        ...
        bool alt, List<BuildSpot> into)
    {
        ...
        //
        // Как только протаскивание задаёт угол, прилипание отключается. С новым
        // поворотом выталкивание уводит первую постройку от точки нажатия, хотя игрок
        // уже выбрал место и крутит только ориентацию: место обязано остаться под
        // курсором. Экстрактор угла не получает, поэтому его притяжение к залежи
        // сохраняется.
        var origin = length >= AngleThreshold && !def.RequiresMetalSpot
            ? anchor
