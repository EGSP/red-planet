...
/// <summary>
/// Слой отрисовки каркаса. Модель каркаса вынесена на собственный узел потому, что материал
...
public partial class BlueprintLayer : Node2D
{
    ...
    /// и переключается на его материал: шейдер строительства обязан действовать на весь
    /// корпус целиком, а материал в Godot есть свойство узла и на потомков само
    ...
    public void AttachModel(UnitDefinition def)
    {
        ...
        // её части отдельно. Меряется до подчинения материалу, но после снятия теней —
    ...
    /// <summary>
    /// Подставить шейдеру настройки и состояние каркаса. Материал создаётся при первом
    /// вызове и принадлежит этому каркасу: доля готовности у каждого своя, поэтому общий
    /// материал на всех не годится.
    /// </summary>
    ...
    public override void _Draw()
    {
        ...
        // его материалу — см. AttachModel. Узел нужен ради материала и порядка, а не рисунка
