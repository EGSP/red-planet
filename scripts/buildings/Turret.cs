...
public partial class Turret : Building, IArmed
{
    ...
    public override void _Draw()
    {
        ...
        // переопределяющая _Draw целиком. Теперь порядок слоёв берётся оттуда же, откуда
        // его берут все остальные, — из BuildingVisual.
        //
        // Основание стоит под углом постановки и вслед за башней не крутится, поэтому
        // поворот ноды снимается и вместо него подставляется угол корпуса
        BuildingVisual.Draw(this, Definition, Vector2.Zero, BodyFacing - Rotation,
            showFootprint: false, modelAttached: Model != null);

        // Дальше — запасной вид. Модель рисует и основание, и башню сама: доворот ствола
        // ей задаёт SyncModel, а поворот ноды до неё не доходит — корпус модели снимает его
        // обратно. Полосу прочности при модели рисует слой пометок, идущий после неё
        if (Model != null)
            return;

        // Башня — треугольник носом вперёд по оси, в координатах ноды, как у конуса
        // прицеливания; поверх непрозрачного основания
