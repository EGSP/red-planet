...
public partial class DebugPanel : CanvasLayer
{
    ...
    /// <summary>
    /// Графика мира: тень препятствий и затенение спрайтов. Вкладка заведена отдельно от
    /// зрения потому, что туман есть часть правил видимости, а эти два вида затенения не
    /// влияют ни на что, кроме вида.
    /// </summary>
    private void FillGfx(Node box)
    {
        Section(box, "Затенение спрайтов",
            "Два запечённых слоя, выведенных из альфы спрайта постройки и посчитанных один " +
            "раз при загрузке. Переключатели гасят слой у всех построек сразу и величины " +
            "в определениях (ao_inner, ao_outer) не трогают.");
        ...
        Check(box, "контактная тень",
            ...
            "плоскости и читается как наклейка. Сила задаётся ключом ao_outer.",
            () => SpriteOcclusion.ContactEnabled, SpriteOcclusion.SetContactEnabled);
        ...
        Check(box, "затемнение по кайме",
            ...
            "корпусу. Сила задаётся ключом ao_inner.",
            () => SpriteOcclusion.RimEnabled, SpriteOcclusion.SetRimEnabled);
