...
/// <summary>
/// Инструмент — то, чем сущность действует на мир: ствол, строительная рука, бур.
///
/// ПОЧЕМУ ОТДЕЛЬНОЕ ОПРЕДЕЛЕНИЕ, А НЕ ПОЛЯ ВНУТРИ ЮНИТА. Раньше строительная мощность,
/// мощность бура и их прожорливость лежали прямо в определении юнита, а дальность была
/// одна на всё — и на бур, и на руку, — тогда как у ствола своя. Инструмент как отдельная
/// вещь снимает и то, и другое: дальность принадлежит тому, кто её применяет, а один и тот же
/// бур можно выдать и копателю, и коммандеру, не повторяя чисел в двух файлах.
///
...
/// <summary>Виды работ, которые инструмент выполняет. Ремонт следует из стройки, см. ниже.</summary>
[System.Flags]
...
/// <summary>
/// Рабочий инструмент: строительная рука, бур или и то и другое.
///
...
public sealed class WorkToolDefinition : ToolDefinition
{
    ...
    /// <summary>Что инструмент умеет. Бур не строит, рука не копает, у коммандера две штуки.</summary>
    ...
    public bool CanMine => Kinds.HasFlag(WorkKinds.Mine);
