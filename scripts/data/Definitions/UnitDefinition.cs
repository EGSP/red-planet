...
public enum UnitClass
{
    ...
    /// <summary>Синтезатор: жжёт энергию и даёт метал.</summary>
    Factory,
...
///
/// РАНЬШЕ БЫЛО ТРИ ОПРЕДЕЛЕНИЯ И РАЗЪЕЗЖАЛИСЬ. Копатель описывался тремя файлами:
...
public sealed class UnitDefinition
{
    ...
    /// <summary>Бур.</summary>
    public WorkToolDefinition MineTool;

    public bool CanBuild => BuildTool != null;

    public bool CanMine => MineTool != null;
    ...
    /// <summary>Инструмент под вид приказа. Копать — буром, всё прочее — рукой.</summary>
    public WorkToolDefinition ToolFor(OrderKind kind) =>
        kind == OrderKind.Mine ? MineTool : BuildTool;

    /// <summary>
    /// На какое расстояние юнит вообще дотягивается работой: сначала рука, потом бур.
    /// Нужно там, где занятие ещё не выбрано, — например, безоружному, которому приказали
    /// атаковать: подойти он должен хотя бы на длину того, чем располагает.
    /// </summary>
    public float WorkRangePx => BuildTool?.RangePx ?? MineTool?.RangePx ?? Const.Unit;
