...
public sealed class UnitDefinition
{
    ...
    /// <summary>
    /// Запечённое изображение вида: сцена, разобранная в плоские данные и образец рабочих
    /// узлов. Печётся при первом рождении машины этого вида и живёт до конца партии —
    /// см. <see cref="ModelBake.For"/>.
    ///
    /// ПОЧЕМУ ХРАНИТСЯ ЗДЕСЬ. Определение вида существует в единственном экземпляре, и все
    /// машины вида ссылаются на него же; отдельный набор запечённых моделей с ключом по пути
    /// к сцене был бы вторым ключом к тем же данным и мог бы с этим разойтись.
    ///
    /// Поле назначает только <see cref="ModelBake"/>: со стороны справочника изображение
    /// задаётся одним лишь ключом <see cref="Model"/>.
    /// </summary>
    public ModelBake Bake;
    ...
    /// уже задан формой, и второе число, обязанное с ней согласовываться, — лишняя возможность
    /// их рассогласовать.
    /// </summary>
    ...
    /// которое всегда вдесятеро больше соседнего, значит заводить лишнюю возможность их рассогласовать.
    /// </summary>
