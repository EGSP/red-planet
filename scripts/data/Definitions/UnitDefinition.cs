...
public sealed class UnitDefinition
{
    ...
    /// <summary>
    /// Путь к спрайту корпуса (<c>res://...</c>). Пустая строка — силуэт рисуется фигурами
    /// по <see cref="Hull"/>. Заданный путь заменяет процедурные фигуры корпуса.
    /// </summary>
    public string Sprite = "";

    /// <summary>
    /// Множитель размера спрайта. У постройки 1 означает вписывание в её габарит, а у
    /// подвижной сущности — исходный размер изображения. Значение больше единицы увеличивает
    /// спрайт относительно соответствующего базового размера.
    /// </summary>
    public float SpriteScale = 1f;

    /// <summary>
    /// Поворот спрайта в градусах относительно локальной оси корпуса (+X вперёд).
    /// </summary>
    public float SpriteRotationDegrees;

    /// <summary>
    /// Сила затемнения по внутренней кайме силуэта — того слоя, что рисуется поверх корпуса
    /// и придаёт ему объём. Ноль отключает слой, единица даёт полную величину, заданную
    /// в <see cref="SpriteOcclusion"/>.
    ///
    /// По умолчанию ноль, поэтому подвижные сущности затенения не получают; постройкам
    /// значение назначает base_building.toml.
    /// </summary>
    public float AmbientOcclusionInner;

    /// <summary>
    /// Сила контактной тени — того слоя, что рисуется под корпусом и отделяет постройку от
    /// поверхности. Величины разделены, поскольку соотношение между тенью на грунте и
    /// затемнением самого корпуса есть художественный выбор и у разных построек может
    /// отличаться.
    /// </summary>
    public float AmbientOcclusionOuter;
