...
/// <summary>
/// План не состоялся: к приходу исполнителя место оказалось занято. Отменяется тихо —
/// очередь идёт дальше, — но след остаётся здесь: молчаливое исчезновение размеченного
/// иначе выглядело бы отказом без причины.
/// </summary>
[TransientEvent]
