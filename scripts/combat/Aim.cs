...
public sealed class AimRig
{
    ...
    /// <summary>
    /// Цель, выбранная носителем самостоятельно, и срок, до которого выбор не переигрывается.
    ///
    /// ЗАЧЕМ ЗАПОМИНАТЬ. Прежде цель, выбранная самим носителем, не запоминалась вовсе:
    /// поиск ближайшей шёл каждый физический шаг у каждого ствола. Полсотни осмотренных
    /// клеток на запрос, помноженные на сотни стволов и на шестьдесят шагов в секунду,
    /// и составляли большую часть стоимости боя. При этом переигрывать выбор шестьдесят раз
    /// в секунду не нужно: цель, выбранная десятую долю секунды назад, остаётся верной.
    ///
    /// Цель, назначенная приказом, сюда не попадает — она принадлежит приказу и живёт
    /// в <see cref="IArmed.FireTarget"/>.
    /// </summary>
    public IDamageable Target;

    /// <summary>
    /// Номер группы, по которому носитель получает свою очередь переигрывать выбор.
    /// Назначается один раз при первом появлении в системе стрельбы; −1 — назначения не было.
    /// </summary>
    public int Group = -1;

    /// <summary>
    /// Разведана ли выбранная цель — ответ, полученный в свою очередь. Разведка обходит всех
    /// носителей обзора стороны и потому квадратична; спрашивать её каждый шаг незачем
    /// по той же причине, по какой не переигрывается сам выбор.
    /// </summary>
    public bool Spotted;
    ...
    /// <summary>
    /// Навести рабочую руку на место работы. Пусто означает, что работы в этом шаге нет,
    /// и рука возвращается к оси корпуса обычным порядком (см. <see cref="Relax"/>).
    ///
    /// ПАРНОЕ ДЕЙСТВИЕ К <see cref="AimWeapons"/>, И ЗАВЕДЕНО ОНО РАДИ ПОСТРОЕК. Наведение
    /// руки прежде было написано внутри <see cref="Unit"/> и существовало только там,
    /// поэтому башня-сборщик, снабжённая точно таким же инструментом на точно таком же
    /// стенде, манипулятор не поворачивала вовсе: наводить его было некому. Ствол такой
    /// беды не знал, потому что его наводит система стрельбы, одна на подвижных
    /// и неподвижных.
    ///
    /// Рука здесь одна — первая по порядку: место работы у носителя одно, как и цель
    /// у стрельбы, а нескольких рук ни у одного вида пока нет.
    /// </summary>
    public void AimWork(float bodyFacing, Vector2 from, Vector2? to, double dt)
    {
        if (to is { } point)
            Aim(PrimaryWork, bodyFacing, from, point, dt);
    }

    /// <summary>
    /// Луч рабочей руки, объявленный в модели. Null означает, что луча в сцене нет: носитель
    /// тогда показывает работу запасным отрезком, как это было до появления лучей.
    /// </summary>
    public BeamVisual WorkBeam => PrimaryWork?.Part?.Beam;

    /// <summary>
    /// Показать луч работы до точки либо погасить его. Зовётся каждый кадр, и пустая точка
    /// означает «работы нет»: гаснет луч сам, с задержкой из своего вида.
    ///
    /// ПОЧЕМУ ЗДЕСЬ, А НЕ У СИСТЕМЫ ЭФФЕКТОВ. Та связывает изображение с ПОВОДАМИ из журнала
    /// — выстрелом, попаданием, гибелью, — а работа документов не порождает и существует
    /// как состояние носителя. Спрашивать это состояние снаружи было бы обходом: носитель
    /// и так закрывает шаг наведения, и точка работы у него в руках.
    /// </summary>
    public void DriveWorkBeam(Vector2? target)
    {
        if (WorkBeam is not { } beam)
            return;
