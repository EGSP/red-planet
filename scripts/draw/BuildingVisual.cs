...
/// <summary>
/// Отрисовка постройки: границы места, площадка, запасной прямоугольник корпуса.
///
/// ЕДИНСТВЕННЫЙ ПОРЯДОК СЛОЁВ ВО ВСЁМ ПРОЕКТЕ. Ею пользуются и <see cref="Building"/>,
/// и <see cref="Turret"/>, и редактор содержимого. Пока последовательность была выписана
/// в каждом из этих мест отдельно, расхождение и случилось: турель переопределяет
/// <c>_Draw</c> целиком и рисовала площадку только в запасной ветке, поэтому с появлением
/// у неё сцены изображения площадка исчезла.
///
/// HealthBar и UnitGizmos сюда не входят — в редакторе прочности нет, а круги дальности
/// рисует Preview сам.
///
...
{
    /// <param name="modelAttached">
    /// Поднята ли сцена изображения у этого экземпляра. Не задано — вопрос решается
    /// по справочнику (<see cref="UnitDefinition.HasModel"/>), как в редакторе, где
    /// экземпляра нет. Игра отвечает по своей ноде: путь в справочнике указан, а сцена
    /// не загрузилась — постройке нужен запасной прямоугольник, иначе она станет невидимой.
    /// </param>
