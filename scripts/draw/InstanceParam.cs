...
/// без создания собственного материала для неё.
///
/// ЗАЧЕМ ЭТО ЗАВЕДЕНО ОТДЕЛЬНЫМ ИНСТРУМЕНТОМ. Прежде цвет команды и доля прочности писались
/// в материал, а чтобы запись не окрасила разом всех носителей вида, каждой машине выдавалась
/// копия <see cref="ShaderMaterial"/>. Godot объединяет двумерные картинки в один вызов
/// отрисовки лишь при совпадении материала, поэтому копии отменяли объединение полностью:
/// при трёх сотнях машин выходило свыше шестисот вызовов. Параметр экземпляра хранится
/// у самого объекта отрисовки, а материал остаётся один на вид.
///
/// ПРАВИЛО ВЫБОРА. Величина принадлежит виду — она живёт обычным <c>uniform</c> в материале
/// сцены. Величина принадлежит экземпляру, а экземпляров в кадре десятки и более — она
/// объявляется в шейдере как <c>instance uniform</c> и пишется отсюда. Копия материала
...
public static class InstanceParam
{
    ...
    ///
    /// ЧТО ИЗМЕРЕНО. Цвет, записанный в материал, доходит до экрана без изменений: движок
    ...
    /// на стенде: 0,42 через материал даёт на экране 0,42, а через параметр экземпляра — 0,149,
    ...
    /// <summary>
    /// Объявляет ли шейдер материала такой параметр экземпляра. Ложь для материала без шейдера
